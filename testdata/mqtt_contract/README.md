...
suites: `cluster_orchestrator/cluster-manager/tests/` and `go_node_engine/clusterlink/`.
...
| `control_deploy.json` | `nodes/<id>/control/deploy` | CM `workerlink.publish_deploy` | NE `deployHandler` | 0 |
| `control_delete.json` | `nodes/<id>/control/delete` | CM `workerlink.publish_delete` | NE `deleteHandler` | 0 |
...
- `control_deploy.json` is the raw Mongo job document plus `instance_number`.
  ...
  Go silently drops.
- CM publishes at QoS 0, NE publishes at QoS 1. Neither side retains. Both facts
  are now enforced by the adapter configuration - the `MqttBus`/`mqtt.Bus` each
  side constructs is given a fixed QoS and never sets the retain flag - rather
  than being a choice made at each individual publish call.
